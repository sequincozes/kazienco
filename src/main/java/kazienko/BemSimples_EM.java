...
//        Instances[]{trainInstances, normalInstances, testInstances, trainInstancesLabeled};
        ads(allInstances[0], allInstances[1], allInstances[2], allInstances[3], 4);
    }

    public static void ads(Instances treino, Instances testeNormal, Instances teste, Instances labeled, int k) throws Exception {
        ...
        int[][][] final_results = new int[k][3][3];

// Criação do cluster (treino)
        int[][] results = new int[k][3];
        EM emClustering = Util.clusterDataEM(treino, k);
...
//            System.out.println(i + "-" + labeled.instance(i).classValue() + " = " + labeled.instance(i).toString());
            int clusterNum = emClustering.clusterInstance(treino.instance(i));
...
//        }
        ...
        if (true) {
            ...
            //Remover esse for até 300 (ERA SÓ PRA CALCULAR TEMPO)
//            for (int temp = 0; temp < 300; temp++) {
            for (int i = 0; i < teste.numInstances(); i++) {
//                    long timeBefore = System.nanoTime();
                int clusterNum = emClustering.clusterInstance(teste.instance(i));
...
//            System.out.println(" ** TESTE NORMAL ** ");
            for (int i = 0; i < testeNormal.numInstances(); i++) {
                int clusterNum = emClustering.clusterInstance(testeNormal.instance(i));
                results[clusterNum][0] = results[clusterNum][0] + 1;
            }
//            for (int i = 0; i < k; i++) {
//                System.out.println("Cluster " + i + "; " + results[i][0]);//+ ";" + results[i][1] + ";" + results[i][2]);
//            }
    ...
    public static void testeComClustering(EM farthestFirst, Instances instancias) throws Exception {
...
//            tempInstance.isMissing(tempInstance.numAttributes() - 1);
            ...
            int clusterNum = farthestFirst.clusterInstance(tempInstance);
